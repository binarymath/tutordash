...
import { BarChart3, CheckCircle2, X, GripHorizontal, Plus } from 'lucide-react';
...
const SortableTurmaCard = ({ turma, onClick, isSelected, onRemove }) => {
  ...
  return (
    <div ref={setNodeRef} style={style} className="relative group">
      <button
        onClick={onClick}
        className={`w-full p-4 rounded-lg border transition-all text-left hover:scale-105 ${turma.completo
      ...
      >
        ...
        <div className="flex gap-2 mb-3">
          {[1, 2, 3, 4].map(num => {
            ...
            return (
              <div
                key={num}
                className={`flex-1 h-2 rounded-full transition-all ${carregado
        ...
        <div className="flex justify-between items-center text-sm">
          ...
          <span className={`font-bold ${turma.completo ? 'text-green-400' : 'text-accent-gold'
            }`}>
...
const DashboardProgresso = () => {
  ...
  return (
    <Card className="bg-brown-900/50 border-brown-700">
      ...
      {mostrarFormulario && (
        <form onSubmit={handleAdicionarTurma} className="mb-6 p-4 bg-brown-800/30 rounded-lg border border-brown-700 animate-slide-down">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2 bg-brown-900/50 border border-brown-600 rounded-lg text-white placeholder-brown-400 focus:outline-none focus:border-accent-gold transition-colors uppercase"
              autoFocus
            />
            <Button
              type="submit"
